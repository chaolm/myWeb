...
public class ModelAndViewAspect {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ModelAndViewAspect.class);

    private static ThreadLocal<String> localId = new ThreadLocal<String>();

    @Pointcut(value = "!@within(org.springframework.web.bind.annotation.RestController) && @annotation(org.springframework.web.bind.annotation.RequestMapping) && !@annotation(org.springframework.web.bind.annotation.ResponseBody) && !@within(org.springframework.web.bind.annotation.ControllerAdvice)")
    ...
    public void afterReturningMethod(JoinPoint jp, Object result){
        String methodName = jp.getSignature().getName();
        logger.info("logId:{}, 【返回通知】the method : {}, return with : {}", localId.get(), methodName, JsonUtil.toFullJson(result));
    }
...
//        String methodName = jp.getSignature().getName();
//        logger.error("【异常通知】the method 【" + methodName + "】 occurs exception: " + e.getMessage(), e);
//        if(e instanceof ServiceException) {
//            throw new RestException(((ServiceException) e).getCode(), e.getMessage());
//        }
